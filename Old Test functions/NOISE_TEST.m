...
%             pi_star_mcla = fromLVtoBA(mclaLV);
    ...
    x_p  = [1,0.9,0.8,0.7,0.6,0.5,0.4,0.3,0.2,0.1,0];
end
figure
p = plot( x_p,NMI_noise(1,:),"-o", x_p, NMI_noise(2,:),"-+",x_p, NMI_noise(3,:),"-*",x_p, NMI_noise(4,:),"-square", x_p, NMI_noise(5,:),"-x");
hold on
l = legend("Hungarian", "Regression", "Matchpairs","Jonker-Volgenant",    "Mcla");
l.Location = "northwest";
title("Average NMI with noise variation")
xlabel("% of correctly labelled points")
ylabel("NMI")
hold off

figure

plot( x_p,ARI_noise(1,:),"-o", x_p, ARI_noise(2,:),"-+",x_p, ARI_noise(3,:),"-*",x_p, ARI_noise(4,:),"-square", x_p, ARI_noise(5,:),"-x");
hold on
l = legend("Hungarian", "Regression", "Matchpairs","Jonker-Volgenant",    "Mcla");
l.Location = "northwest";

title("Average ARI with noise variation")
xlabel("% of correctly labelled points")
ylabel("ARI")
hold off
...
figure
p = plot( x_p,NMI_noise_gt(1,:),"-o",  x_p, NMI_noise_gt(2,:),"-+",x_p, NMI_noise_gt(3,:),"-*",x_p, NMI_noise_gt(4,:),"-square", x_p, NMI_noise_gt(5,:),"-x");hold on
p(1).MarkerSize = 6;
p(2).MarkerSize = 6;
p(3).MarkerSize = 10;
p(4).MarkerSize = 20;
...
title("Average NMI wrt the GT with noise variation")
xlabel("% of correctly labelled points")
ylabel("NMI")
hold off
...
plot( x_p,ARI_noise_gt(1,:),"-o", x_p, ARI_noise_gt(2,:),"-+",x_p, ARI_noise_gt(3,:),"-*",x_p, ARI_noise_gt(4,:),"-square", x_p, ARI_noise_gt(5,:),"-x");hold on
hold on
l = legend("Hungarian", "Regression", "Matchpairs","Jonker-Volgenant",    "Mcla");
l.Location = "northwest";

title("Average ARI wrt the GT with noise variation")
xlabel("% of correctly labelled points")
ylabel("ARI")
hold off
